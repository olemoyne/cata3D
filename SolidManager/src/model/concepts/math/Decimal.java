...
public class Decimal implements Comparable<Decimal>, Serializable{
	...
	public static final MathContext ROUNDING_VECTOR_CTX = new MathContext(5, RoundingMode.HALF_UP);
...
//			System.out.println("Error dividing : "+this.toString()+" with "+d.toString());
	...
	public long longVectorValue() {
		BigDecimal rnd = dec.round(ROUNDING_VECTOR_CTX);
		return rnd.longValue();
	}
	...
	public Decimal aSin() {
		if (dec == null) dec = new BigDecimal(val);
		double db = Math.asin(dec.doubleValue())*180d/Math.PI;
		if (Double.isNaN(db)) 
			return ZERO;
