...
public class MapDeVecteurs implements Serializable {
	...
	public MapDeVecteurs truncate(Plan3D surface) {
		...
		// Structure de stockage du r�sultat
		int nextPos = 0;
		...
		for (int ypos = 0; ypos < this.ySize; ypos ++) {
			...
			if (isFull) {
				if (surface.donneCote(getPoint(0, ypos)) > 0) {// en dessous
					// on int�gre tous les pts dans le r�sultat
					for (int xpos = 0; xpos < this.xSize; xpos ++) {
					...
					if (surface.donneCote(v) > 0) {// en dessous
						...
						if (inter == null) {
							if (xpos > 0) {
								...
								// Parcours tous les points d�j� parcourus pour setter l'inter
...
**/		
		...
		// Aucune position au dessous ou � la surface
	...
	// D�coupe en tranche sur le plan X = pos
	public Area intersectionHorizontaleX(Plan3D pl) {
		...
		for (int ypos = 0; ypos < this.ySize; ypos ++) {
			...
			if (!ok) { // haut et bas non affect�s
		...
		// parcous les liste et ajout les �l�ments
	...
	/**
	 * Retourne la projection de la forme sur un plan donné par l'axe
	 *  
	 * @return
	 */
