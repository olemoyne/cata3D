...
public class PrintedQuille extends PrintableObject {

	public Area contours;
	public Area interieur;

	
	public Decimal epaisseur;
	public Decimal profondeur;
	
	public PatchVide boat;
	
	public Bounds bns;
	
	public Segment mer;
	
	ArrayList<Vecteur> limiteBordure; 
	...
	/** construit le dessin de la quille **/
	public Area computeForm() {
		// Construit le plan 3D de la quille
		Plan3D pl = new Plan3D(new Vecteur(epaisseur, Decimal.UN, Decimal.ZERO), new Vecteur(epaisseur, Decimal.ZERO, Decimal.ZERO), new Vecteur(epaisseur, Decimal.UN, Decimal.UN));
		contours = boat.mapAffichage.CreateProjection(pl);
		
		// Forme d'intérieur
		interieur = reduceNormal(contours, this.profondeur, Axis.XAxis);
		
		// TODO : Epaisseur sur les cotés et sur le devant
		
		// POint les plus bas des gabarits
		limiteBordure = new ArrayList<Vecteur>(); 
		for (Gabarit g : this.boat.structure.gabarits) {
			// Identifie le point le plus bas
			Area a = g.getArea(boat, g.position, null, false);
			Vecteur pt = a.points.get(0);
			for (Vecteur v : a.points) {
				if (v.getY() < pt.getY()) pt = v;
			}
			limiteBordure.add(pt);
		}
